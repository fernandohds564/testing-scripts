...
def getPVsInfo(session, PV_names):
    if isinstance(PV_names, (list, tuple)):
        PV_names = ','.join(PV_names)
    return session.get(URL + '/getPVStatus?pv=' + PV_names)

def getAllPVs(session, PV_names):
    if isinstance(PV_names, (list, tuple)):
        PV_names = ','.joint(PV_names)
    return session.get(URL + '/getAllPVs?pv=' + PV_names+'&limit=-1')
