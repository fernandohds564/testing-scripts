...
class ObstructionStudy(DataBaseClass):
    ...
    def plot_lost_element_histogram(self):
        ...
        lat = fig.add_subplot(gs_[1, 0], sharex=axis_lelem)
        pyaccel.graphics.draw_lattice(self.model, gca=lat)

        # lpos += 1e-2*np.random.rand(lpos.size)
        axis_lelem.hist(
            lpos,
            bins=int(self.model.length / 3)
            )
...
def main():
    ...
    obs.model.cavity_on = True
    idx = pyaccel.lattice.find_indices(
        obs.model, 'frequency', 3, comparison=lambda x, y: x > y)[0]
    obs.model[idx].voltage = 1.75e6

    obs.params.nparticles = 2000
    obs.params.nturns = 800
    obs.params.measurement_type = 'dynap'
    obs.params.obstruction_size = 0e-3
    obs.params.obstruction_side = 'negative'
    obs.params.dynap_kick = -5.8e-3 * 0
    obs.params.injection_position = -8.4e-3 * 0
    obs.params.delta_energy = -0.1/100 * 0
    ...
    # obs.do_study()
    # obs.save_data(
    #     obs.params.suggest_name(), overwrite=True)
